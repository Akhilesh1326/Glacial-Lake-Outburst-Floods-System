...
const SignUp = () => {
  ...
  const handleSubmitSignUpData = async() =>{
    try{
        const resp = await axios.post("https://glacial-lake-outburst-floods-system.onrender.com/api/emergency-resp-validation/data",{
          orgnization,
          areaOfOperation,
          accessLevel,
          governementIdProof,
  ...
  return (
    <div className='flex flex-col justify-center items-center bg-slate-900 text-slate-200 h-screen'>
        ...
        <div className='flex flex-col text-lg font-light text-slate-200'>
          <div>Orgnization</div>
          <input type="text" placeholder='Enter Name' value={orgnization} onChange={(e)=>setOrgnization(e.target.value)}className="bg-slate-600 py-2 px-4 mx-5 my-2 rounded-lg"/>
          <div>Area Of Operation</div>
          <input type="text" placeholder='Enter Name' value={areaOfOperation} onChange={(e)=>setAreaOfOperation(e.target.value)}className="bg-slate-600 py-2 px-4 mx-5 my-2 rounded-lg"/>
